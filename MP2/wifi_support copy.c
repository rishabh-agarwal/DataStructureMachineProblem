...
void wifi_add(List list_ptr, int wifi_id, int size)
{
 ...
 /*  else if(comp_proc(rec_ptr->eth_address,wifi_id))//Replace record
   {
    list_insert_sorted(list_ptr, rec_ptr);
 ...
 /*else if(list_size(list_ptr)==size) // Error with adding record
...
{   
    ...
    wifi_record_fill(idx_ptr->data_ptr);
    list_insert(list_ptr,idx_ptr->data_ptr,idx_ptr);
