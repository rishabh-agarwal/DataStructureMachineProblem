 ...
 * -s 12345  random number generator seed
 * -v        verbose
 *
 ...
 *
 *  An optional feature to change the search policy:
...
// Global variables for optional features in mem.h
...
int main(int argc, char **argv)
{
 ...
 else if (dprms.UnitDriver == 2)
    {
        ...
        int num_bytes_3;
        // allocate 1st pointer
        num_bytes_1 = 511*unit_size;
        p1 = (int *) Mem_alloc(num_bytes_1 );
        printf("first allocation of %d bytes p=%p \n", num_bytes_1, p1);
        Mem_print();
        // allocate for 2nd pointer
        num_bytes_2 = 511*unit_size;
        p2 = (int *) Mem_alloc(num_bytes_2);
        printf("second allocation of %d bytes p=%p\n", num_bytes_2, p2);
        Mem_print();
        // allocate for 4th pointer when free list is empty
        num_bytes_3 = 511*unit_size;
        p3 = (int *) Mem_alloc(num_bytes_3);
        printf("fourth allocation of %d bytes p=%p\n", num_bytes_3, p3);
        Mem_print();
        // the above Mem_print should show the free list as empty
     ...
     else if (dprms.UnitDriver == 3)
    {
        ...
        int num_bytes_3;
        // allocate 1st pointer
        num_bytes_1 = 599*unit_size;
        p1 = (int *) Mem_alloc(num_bytes_1 );
        printf("first allocation of %d bytes p=%p \n", num_bytes_1, p1);
        Mem_print();
        // allocate for 2nd pointer
        num_bytes_2 = 119*unit_size;
        p2 = (int *) Mem_alloc(num_bytes_2);
        printf("second allocation of %d bytes p=%p\n", num_bytes_2, p2);
        Mem_print();
        // allocate for 4th pointer when free list is empty
        num_bytes_3 = 19*unit_size;
        p3 = (int *) Mem_alloc(num_bytes_3);
        printf("fourth allocation of %d bytes p=%p\n", num_bytes_3, p3);
        Mem_print();
        // the above Mem_print should show the free list as empty
...
void equilibriumDriver(driver_params *ep)
{
    ...
    for (i = 0; i < ep->WarmUp; i++) {
        ...
        for (index = 1; index < size; index++)
            ptr[index] = -index;
        list_insert(mem_list, (data_t *) ptr, list_iter_tail(mem_list));
        ptr = NULL;
    }
    printf("After warmup\n");
    if (!ep->SysMalloc) {
