...
{
    assert(strcmp(list_type, "List")==0 || strcmp(list_type, "Queue" )==0 ||strcmp(list_type, "SQueue" )==0);
    pIterator index;
    int counter = 0;

    index = list_iter_first(list_ptr);
    pIterator index_tail=list_iter_tail(list_ptr);
    while (index != index_tail->next)
    {   
        printf("%d: ", ++counter);
        hpot_record_print(list_access(list_ptr, index));
        index = index->next;//list_iter_next(index);
    }
    if (counter == 0) {
        printf("%s is empty\n", list_type);
    } else {
...
//--------------------------APPENDL-----------------------------------------//
void hpot_tail(pList list_ptr,int rec)
{    
    packet_t * new_node;
    new_node=(packet_t*)malloc(sizeof(packet_t));
    new_node->dest_ip_addr=rec;
...
{   int pkt_id=0;
    ...
    pkt_id=list_access(list_ptr,list_iter_first(list_ptr))->dest_ip_addr; 
...
{   
     ...
     if(rover!=NULL)
     {
      while(rover != list_iter_tail(list_ptr))
      {
       ...
       {
       count ++;
       hpot_record_print(list_access(list_ptr,rover));
...
//---------------------------SCANL----------------------------------------//
void hpot_scanl(pList list_ptr, int thresh)
{
    int count = 0;
    int found_addr = -1;
    int sets = 0;
 ...
 */
//-----------------------------------------------------------------------//
void hpot_stats(pList sorted, pList unsorted, pList un_sorted)
{
    // get the number in list and size of the list
    int num_in_sorted_list = 0;
    int num_in_unsorted_list = 0;
    int num_in_un_sorted_list=0;
    
    num_in_sorted_list=list_size(sorted);
    num_in_unsorted_list=list_size(unsorted);
    num_in_un_sorted_list=list_size(un_sorted);
    
    printf("Number records in list: %d, queue size: %d,third list Size: %d\n", 
            num_in_sorted_list, num_in_unsorted_list ,num_in_un_sorted_list);
}
//-----------------------------------------------------------------------//
